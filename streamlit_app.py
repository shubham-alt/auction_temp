...
def finalize_auction():
    ...
    if current_bid["winner"]:
        ...
        st.session_state.auction_history.append({"player": current_player, "price": current_bid["price"], "winner": winner})
        st.success(f"{current_player['Name']} sold to {winner} for ₹{current_bid['price']} crore.")
    else:
        st.session_state.auction_history.append({"player": current_player, "price": 0, "winner": "Unsold"})
...
if st.session_state.current_player:
    ...
    st.write(f"**Time Remaining:** {int(get_remaining_time())} seconds")
else:
    st.write("No player is currently being auctioned.")
